...
async def add_monitor(ctx, product_sku: str):
    monitor_process = Popen(f'python -c "from yeezysupplymonitor import YeezySupplyMonitor; monitor = YeezySupplyMonitor(); monitor.monitor_site(\\"{product_sku}\\", 5)"', shell=True)
